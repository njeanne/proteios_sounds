...
def get_uniprot_features(entry, protein_dict):
    ...
    for feature in entry.features:
        ...
        # Modified residue: Modified residues excluding lipids, glycans and
        # protein_dict crosslinks
        if feature[0] == 'MOD_RES':
            modif = re.split(' {', feature[3])[0].rstrip('.;,')
            if 'modified_residue' not in protein_dict.keys():
                protein_dict['modified_residue'] = {modif: [(feature[1],
                                                             feature[2])]}
            else:
                if modif in protein_dict['modified_residue'].keys():
                    protein_dict['modified_residue'][modif].append((feature[1],
                                                                    feature[2]))
                else:
                    protein_dict['modified_residue'][modif] = [(feature[1],
                                                                feature[2])]
        ...
        if feature[0] == 'CARBOHYD':
            ...
            if 'glycosylation' not in protein_dict.keys():
                protein_dict['glycosylation'] = {glyco: [(feature[1],
                                                          feature[2])]}
            else:
                if glyco in protein_dict['glycosylation'].keys():
                    protein_dict['glycosylation'][glyco].append((feature[1],
                                                                 feature[2]))
                else:
                    protein_dict['glycosylation'][glyco] = [(feature[1],
                                                             feature[2])]
        ...
        if feature[0] == 'SITE':
            ...
            if 'site' not in protein_dict:
                ...
                if site in protein_dict['site']:
                    ...
                    protein_dict['site'][site] = [(feature[1], feature[2])]
        # Propeptide: Part of a protein that is cleaved during maturation
        # or activation
        if feature[0] == 'PROPEP':
            propep = re.split(' {', feature[3])[0].rstrip('.;,')
            if 'propeptide' not in protein_dict:
                protein_dict['propeptide'] = {propep: [(feature[1],
                                                        feature[2])]}
            else:
                if propep in protein_dict['propeptide'].keys():
                    protein_dict['propeptide'][propep].append((feature[1],
                                                               feature[2]))
                else:
                    protein_dict['propeptide'][propep] = [(feature[1],
                                                           feature[2])]
...
def parse_entry(uniprot_AN, midi_keys):
    ...
    try:
        ...
        if http_err.code == 404:
            err_msg = ('{} {} accession number does not exists in UniProt '
                       'database, check https://www.uniprot.org/ to get the '
                       'correct accession number.').format(http_err, uniprot_AN)
    ...
    if pdb_acc_num:
        ...
        logging.info(('PDB: {} (Protein DataBase accession number, see: '
                      'http://www.rcsb.org/)').format(pdb_acc_num))
    else:
