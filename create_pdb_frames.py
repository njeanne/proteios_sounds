...
if __name__ == "__main__":
    ...
    parser.add_argument("--color_aa", required=False, action="store_true",
    ...
    pymol.stored_list = []
    pymol.cmd.iterate(f"(name ca) and (chain {args.chain})", "pymol.stored_list.append((resi, oneletter))")
    pymol.cmd.hide("all")
    pymol.cmd.show("cartoon")
    pymol.cmd.set("ray_opaque_background", 1)

    if args.color_aa:
        pymol.cmd.color("red", f"resi {args.idx_aa}")
    img_path = os.path.join(args.pdb_dir, "frames", f"{args.pdb_AN}_{args.frame_nb}.png")
    logging.info(f"[Pymol] Frame {args.frame_nb} (in PDB file): {img_path}")
