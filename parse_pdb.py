...
def get_common_coordinates(pdb_seq, uniprot_seq, pdb_coord_matching):
    ...
    :param pdb_coord_matching: the dictionary
    :type pdb_coord_matching: dict
    :param pdb_seq: the PDB sequence.
    :type pdb_seq: str
    :param uniprot_seq: the UniProt sequence.
    :type uniprot_seq: str
...
def get_pdb_info(prot_dict, pdb_dir):
    """
    Get information about PDB sequences.

    :param prot_dict: the protein dictionary.
    :type prot_dict: dict
    :param pdb_dir: the path of the pdb data directory.
    :type pdb_dir: str
    :return: the PDB data
    :rtype: dictionary
    """

    logging.info(f"Fetching PDB file with accession number: {prot_dict['PDB']}")
    ...
    frames_idx = [i + pdb_data["PDB_start_on_uniprot"] for i in
                  range(pdb_data["PDB_stop_on_uniprot"] - pdb_data["PDB_start_on_uniprot"] + 1)]
